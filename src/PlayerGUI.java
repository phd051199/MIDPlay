...
public class PlayerGUI implements PlayerListener {
  ...
  private void changeTrack(boolean forward) {
    ...
    isLoading = false;
    if (isShuffleEnabled) {
      changeTrackShuffle(forward);
    } else {
      changeTrackNormal(forward);
    }
    closePlayer();
    play();
  }

  private void changeTrackNormal(boolean forward) {
    Track[] tracks = trackList.getTracks();
    if (forward) {
      currentTrackIndex++;
  ...
  private void changeTrackShuffle(boolean forward) {
  ...
  private boolean hasNextTrack() {
    ...
    if (isShuffleEnabled) {
      ...
      return currentTrackIndex < trackList.getTracks().length - 1;
    }
  }
