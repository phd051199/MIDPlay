    ...
    try {
      ...
      if (message != null) {
        if (message.indexOf("Connection reset") != -1) {
          throw new IOException("Network connection failed: " + message);
        } else if (message.indexOf("timeout") != -1 || message.indexOf("Timeout") != -1) {
          throw new IOException("Network operation timed out: " + message);
        }
      }
      throw new IOException("Network error: " + message);
    } finally {
